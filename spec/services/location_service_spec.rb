    ...
    it 'Can return a list of nearby atms from a lat/lon' do
      VCR.use_cassette('atms_near_me') do
        ...
        atms_results.each do |atm|
          ...
          expect(atm[:address]).to have_key(:streetNumber)
          expect(atm[:address]).to have_key(:streetName)
          expect(atm[:address]).to have_key(:municipality)
          expect(atm[:address]).to have_key(:countrySubdivision)
          expect(atm[:address]).to have_key(:postalCode)
